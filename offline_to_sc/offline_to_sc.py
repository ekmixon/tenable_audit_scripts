...
def read_file(filename):
    ...
    try:
        ...
        display(f'ERROR: read_file(): reading file: {filename}: {e}', exit=1)
...
def write_file(filename, content, overwrite=False):
    if os.path.isfile(filename) and not overwrite:
        display(f'ERROR: write_file(): file exists: {filename}', exit=1)
    ...
    try:
        ...
        display(f'ERROR: write_file(): writing file: {filename}: {e}', exit=1)
...
def get_host_properties_from_nessus(contents):
    ...
    try:
        ...
        display(f'ERROR: get_host_properties_from_nessus(): {e}', exit=1)
        sys.exit(1)

    display(f'Host Name: {name}', verbose=True)
...
def create_filename(source, override):
    ...
    if not filename:
        ...
        ext = source.split('.')[-1]
        filename = f'{basefile}.offline_import.{ext}'

    display(f'Using filename of {filename}', verbose=True)
...
def apply_values_to_nessus(contents, values):
    ...
    try:
        ...
        for host in values:
            display(f'Apply values: {host}', verbose=True)
            ...
            for report_host in report_hosts:
                ...
                display(f'Analyzing report: {report_name}', verbose=True)
                if report_name.lower() == name.lower():
                    display(f'Found report name: {name}', verbose=True)
        ...
        display(f'ERROR: apply_values_to_nessus(): {e}', exit=1)
        sys.exit(1)

    new_content = ET.tostring(tree, encoding='ascii', method='xml', short_empty_elements=False)

    return sanitize_xml_to_nessus(new_content)
...
def sanitize_xml_to_nessus(source):
    ...
    for value in finds:
        ...
        content = content.replace(f'>{value}<', f'>{replace}<')
